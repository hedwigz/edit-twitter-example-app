...
func (tq *TweetQuery) QueryLikes() *UserQuery {
	query := &UserQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (tq *TweetQuery) WithLikes(opts ...func(*UserQuery)) *TweetQuery {
	query := &UserQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TweetQuery) sqlAll(ctx context.Context) ([]*Tweet, error) {
	...
	if query := tq.withLikes; query != nil {
		...
		ids := make(map[int]*Tweet, len(nodes))
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(tweet.LikesPrimaryKey[0], fks...))
			},
