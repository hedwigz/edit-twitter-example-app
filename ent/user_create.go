...
func (uc *UserCreate) AddLikedIDs(ids ...int) *UserCreate {
	uc.mutation.AddLikedIDs(ids...)
	return uc
}

// AddLiked adds the "liked" edges to the Tweet entity.
func (uc *UserCreate) AddLiked(t ...*Tweet) *UserCreate {
	ids := make([]int, len(t))
	for i := range t {
		ids[i] = t[i].ID
	}
	return uc.AddLikedIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.LikedIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
