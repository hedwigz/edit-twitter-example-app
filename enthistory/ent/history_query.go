...
func (hq *HistoryQuery) QueryChanges() *ChangesQuery {
	query := &ChangesQuery{config: hq.config}
...
func (hq *HistoryQuery) Clone() *HistoryQuery {
	...
	return &HistoryQuery{
		...
		withChanges: hq.withChanges.Clone(),
...
func (hq *HistoryQuery) WithChanges(opts ...func(*ChangesQuery)) *HistoryQuery {
	query := &ChangesQuery{config: hq.config}
	for _, opt := range opts {
		opt(query)
	}
	hq.withChanges = query
	return hq
}
...
//		GroupBy(history.FieldEntityName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.History.Query().
//		Select(history.FieldEntityName).
//		Scan(ctx, &v)
//
...
func (hq *HistoryQuery) sqlAll(ctx context.Context) ([]*History, error) {
	...
	if query := hq.withChanges; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*History)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Changes = []*Changes{}
		}
		query.withFKs = true
		query.Where(predicate.Changes(func(s *sql.Selector) {
			s.Where(sql.InValues(history.ChangesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.history_changes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "history_changes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "history_changes" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Changes = append(node.Edges.Changes, n)
		}
	}
