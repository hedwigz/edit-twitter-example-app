...
// The ChangesFunc type is an adapter to allow the use of ordinary
// function as Changes mutator.
type ChangesFunc func(context.Context, *ent.ChangesMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ChangesFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ChangesMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ChangesMutation", m)
	}
	return f(ctx, mv)
}
